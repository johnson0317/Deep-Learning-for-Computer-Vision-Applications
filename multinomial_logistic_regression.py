...
class MultinomialLogisticRegression(object):
    ...
    def softmax(self,x):
        ...
        for row in x:
            maxi = max(row)
            temp = list(map(lambda x : np.exp(x - maxi), row))
            temp /= np.sum(temp)
            probs.append(temp)
        #############################################################################
        #                              END OF YOUR CODE                             #
    ...
    def predict(self, X):
        ...
        #############################################################################
        predictions = X.dot(self.params['W'] + self.params['b'])
    ...
    def loss(self, X, y=None, lambda_reg=0.0):
        ...
        probs = []
        for l_index, label in enumerate(y):
            probs.append(score[l_index][label])
        ...
        #############################################################################
        loss = (np.sum(-np.log(probs))) / N + lambda_reg * np.sum(W**2)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        ...
        #############################################################################  
        score[range(N), y] -= 1
        grads['W'] = 1/N * X.T@score + lambda_reg * 2 * W
        grads['b'] = 1/N * np.sum(score, axis=0)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
