...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for test_index in range(num_test):
            arr_test = X[test_index]
            for train_index in range(num_train):
                arr_train = self.X_train[train_index]
                dists[test_index, train_index] = np.linalg.norm(arr_test - arr_train)
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            #########################################################################
            arg_sort_arr = np.argsort(dists[i])
            for top_k_ele in arg_sort_arr[:k]:
                closest_y.append(self.y_train[top_k_ele])
            bin = np.bincount(np.array(closest_y))
            y_pred[i] = np.argmax(bin)
            #########################################################################
            #                         END OF YOUR CODE                              #
            #########################################################################
    ...
    def compute_distances_one_loop(self, X):
        ...
        #######################################################################
        for test_index in range(num_test):
            minus_matrix = self.X_train - X[test_index]
            dists[test_index] =  np.linalg.norm(minus_matrix, axis = 1)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def compute_distances_no_loops(self, X):
        ...
        #######################################################################
        dists = -2 * np.dot(X, self.X_train.T) + np.sum(np.square(self.X_train), axis=1)[np.newaxis, :] + np.sum(np.square(X), axis=1)[:, np.newaxis]
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
