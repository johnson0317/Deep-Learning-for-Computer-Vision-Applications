...
class RegularizedRegression(object):
    ...
    def poly_feature_transform(self,X,poly_order=1):
        ...
        """
        f_transform = X
        data_num = f_transform.shape[0]
        #############################################################################
        # TODO: Transform your inputs to the corresponding polynomial with order    #
        # given by the parameter poly_order.                                        #
        #############################################################################
        ones_arr = np.ones(data_num)
        for order in range(poly_order):
            if order == 0:
                sample = f_transform
            else:
                order_matrix = sample ** (order + 1)
                f_transform = np.concatenate((f_transform.T, order_matrix.T)).T
        f_transform = np.concatenate((f_transform.T, ones_arr[np.newaxis, :])).T
    ...
    def train_analytic(self, X, y, poly_order=1, lambda_reg=0):
        ...
        #############################################################################
        X = self.poly_feature_transform(X, poly_order = poly_order)
        X_dot = X.T.dot(X) 
        y_dot = X.T.dot(y)
        self.params['W'] = np.linalg.pinv(X_dot + lambda_reg * np.identity(poly_order + 1)).dot(y_dot)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def initialize_weights(self, dim, std_dev=1e-2):
        ...
        #############################################################################
        self.params['W'] = (np.random.randn(dim) * std_dev)[:, np.newaxis]
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################        
    ...
    def loss(self, X, y=None, lambda_reg=0.0):
        ...
        #############################################################################
        loss = (sum(np.square(prediction - y)) / (2 * N)) + (lambda_reg / 2) * (sum(np.square(W)))
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        grads = {}
        ...
        grads['W'] = ((error.dot(X)) / N).T + lambda_reg * W
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def train(self, X, y, poly_order=1, learning_rate=0.2, lambda_reg=0, num_iters=100, std_dev=1e-2,
        ...
        for it in range(num_iters):
            ...
            #self.params['W'] * (1 - lambda_reg / batch_size) - (learning_rate / batch_size) * grads['W'] 
            #learning_rate * (grads['W'] + 2 * lambda_reg * self.params['W'])
            #self.params['W'] * (1 - lambda_reg / batch_size) - (learning_rate / batch_size) * grads['W']
            #self.params['W'] - learning_rate * grads['W'] - lambda_reg * self.params['W']
            #########################################################################
            #                             END OF YOUR CODE                          #
            #########################################################################
